...
interface GenerateTypesOptions {
  ...
  // File Discovery
  include?: string[]         // Glob patterns for locale files
  exclude?: string[]         // Glob patterns to exclude

  // Output Configuration
  typesPath?: string         // Path for TypeScript definitions
  constsPath?: string        // Path for runtime constants
  virtualFilePath?: string   // Path for virtual module (debugging)

  // Locale Configuration
  baseLocale?: string        // Primary locale (default: 'de')
  merge?: 'deep' | 'shallow' // Merge strategy (default: 'deep')

  // Virtual Module
  virtualId?: string         // Virtual module ID
  sourceId?: string          // Source module ID

  // Customization
  banner?: string            // Custom header comment
  getLocaleFromPath?: (path: string, root: string) => string | null
  transformJson?: (json: unknown, path: string) => unknown

  // Options
  exportMessages?: boolean   // Export messages in generated file
...
async function generateTypes() {
  try {
    const result: GenerateTypesResult = await generateI18nTypes({
      ...
      constsPath: 'src/types/i18n.gen.ts',
