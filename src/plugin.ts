...
export function vitePluginVueI18nTypes(
  ...
  let runtimeModuleCode = 'export {}'
  let messagesModuleCode = 'export {}'
  ...
  return {
    ...
    async configResolved(cfg) {
      ...
      rebuildManager = new RebuildManager({
        ...
        onRebuildComplete: (cache) => {
          ...
          runtimeModuleCode = cache.runtimeModule.js
          messagesModuleCode = cache.messagesModule.js
    ...
    async buildStart() {
      ...
      runtimeModuleCode = result.runtimeModule.js
      messagesModuleCode = result.messagesModule.js
    ...
    load(id) {
      if (id === config.resolvedVirtualJsonId) {
        logger.info(`📄 [load] Loading messages module: ${id}`)
        return {
          code: messagesModuleCode,
          map: null,
        }
      }

      if (id === config.resolvedVirtualId) {
        logger.info(`📄 [load] Loading runtime module: ${id}`)
        return {
          code: runtimeModuleCode,
          map: null,
        }
      }
