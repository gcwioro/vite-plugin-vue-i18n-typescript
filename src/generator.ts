...
import {getFinalKeys, getJsonLeafPaths} from './utils/json'
...
export function toTypesContent(params: Omit<DtsContentParams<JSONObject>, 'typeFilePath'>): string {
  ...
  const messagesJson = JSON.stringify(messages as JSONObject)
...
`.split('\n').map(l => l.trim()).filter(l => l).join('\n') + '\n\n')
  ...
  const bodyLines = [
    ...
    getMessageTypeDefinitions(baseLocale, messages, AllSupportedLanguages).split(NL).map(l => '  ' + l).join(NL),
...
export function getMessageTypeDefinitions(baseLocale: string, messages: Record<string, JSONObject>, AllSupportedLanguages: string[]): string {
...
export function toVirtualModuleContent(params: {
  ...
  const {messages, baseLocale} = params;
