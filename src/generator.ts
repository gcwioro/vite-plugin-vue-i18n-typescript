...
// Content-Hash: ${contentId}
/// <reference types="${typeFilePath.replace(/\.d\.ts$/, '')}" />
`)
  ...
  const bodyLines = [
    ...
    `} from  '${sourceId}'`, //'${typeFilePath.replace(/\.d\.ts$/, '')}'`,
    '',
    `export const supportedLanguages = ${languagesTuple} as const satisfies SupportedLanguagesGen`,
...
export function toVirtualModuleContent(params: {
  ...
  const contentId = fnv1a32(messagesJson);

//   const autogenerated = banner ??
//     `// AUTO-GENERATED FILE. DO NOT EDIT.
// // Generated by unplugin-vue-i18n-dts-generation (virtual module)
// // Content-Hash: ${contentId}
// `;
//
//   const NL = '\n';
...
export function createVirtualModuleCode(opts: {
  ...
  const {jsonText, sourceId, baseLocale, exportData, buildAssetRefId, devUrlPath} = opts;
...
${helperMethodsDefinition(baseLocale, finalKeys).join('\n')}

//inCreateVirtualModuleCode
`;
}


function helperMethodsDefinition(baseLocale: string, finalKeys: string[]) {
  return [
    "//import type {I18n, I18nOptions} from 'vue-i18n'",
    "// import { type Plugin} from 'vue'",
    'import {',
